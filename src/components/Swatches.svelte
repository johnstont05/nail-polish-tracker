<script>
    ...
    const sheetUrl = 'https://api.sheetbest.com/sheets/15473abe-cdd3-4f73-8112-ae74a8f75f32';
    ...
    onMount(async () => {
      ...
      const colors = new Set(polishes.map(p => p.color?.toLowerCase()).filter(Boolean));
      colorCategories = ['all', ...Array.from(colors)];
    });
  
    // Filter the polishes by selected color category
    $: filteredPolishes = selectedColor === 'all'
      ? polishes
 ...
 <div class="filter">
  <label for="color-filter">Filter by color group:</label>
